...
public class EWCFG extends JSONConfig {
    ...
    public EWCFG() {
        super(((Supplier<Path>) () -> {
            ...
            return Paths.get(ViciousDirectories.configDirectory.toAbsolutePath() + "/experiencedworlds.json");
        }).get());
    }
    public ConfigurationValue<Double> advancementMultiplierBase = add(new ConfigurationValue<>("AdvancementMultiplierBase", ()->0.05,this).description("The multiplier base for advancements"));
    public ConfigurationValue<Boolean> multipliersExponentialGain = add(new ConfigurationValue<>("MultiplierExponentialGain", ()->true,this).description("If true the multiplier gained per achievement will be equal to advancementMultiplierBase^numAdvancements, if false: advancementMultiplierBase*numAdvancements"));
